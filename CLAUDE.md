...
8. **Envio de email está configurado e testado em produção** (2026-09-24): SMTP = `mail.simuresi.com.py:465` (SSL/TLS) com a conta `no-responder@simuresi.com.py`, salvo criptografado via dashboard admin. `pagos@simuresi.com.py` e `soporte@simuresi.com.py` são caixas reais próprias (não reenviadores) usadas como `paymentsEmail`/`supportEmail`. Fluxo de recuperação de senha confirmado ponta a ponta (email chegou de verdade). Se `src/services/mailer.js` voltar a fazer no-op silencioso, é porque `smtpHost`/`smtpUser`/`smtpPass` sumiram do Setting `email` — conferir com `emailSettings.get()` (ver "Diagnóstico Rápido").
